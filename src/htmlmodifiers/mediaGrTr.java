...
public class mediaGrTr {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while(scannerMediaMap.hasNextLine()){   
            ...
            //if the first two letters aren't tr, then go to the next line
            if(str.length()<2||!str.substring(0,2).equals("tr")){continue;}
            ...
            //tr.d035.trMA.Exaposteilarion1.c12
            ...
            currentLine[2] = replaceService(dayDesc, currentLine[2], specialTr);
            currentLine[3] = replaceKey(keyDesc,currentLine[3], specialTr+serviceIdentifier);
            
            //sets the beginning of the tree
            //This will only get accessed for the first iteration that finds an "tr"
            if(firstLine) {
            	firstLine = false;
        ...
        try {
            FileWriter myWriter = new FileWriter("C:/git/ages-alwb-system/net.ages.liturgical.workbench.system/MEDIA_INDEX_UTILITY/output/grTR.html");
            myWriter.write(fs);
            myWriter.close();
    ...
    public static String replaceKey(ArrayList<String> arr, String str, String id) { //str is level4 -- arr is key desciriptor
        ...
        for(int j = 0; j<arr.size(); j++){
            if(Pattern.matches("^tr"+id+".*[.]"+str+"[.].*",arr.get(j))){  //checking key descriptor
