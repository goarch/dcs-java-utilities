...
public class mediaGrOc {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while(scannerMediaMap.hasNextLine()){   
            ...
            //if the first two letters aren't oc, then go to the next line
            if(str.length()<2||!str.substring(0,2).equals("oc")){continue;}
            
            if(str.contains(".MM.")) {str = str.replace(".MM.", ".");} //get rid of all .MM.
            if(str.contains(".octr")) {str = str.replace(".octr", ".oc"); specialTr = "tr";}else {specialTr = "";} //get rid of special tr
            ...
            //oc.m8.d1.ocMA.Antiphon41.c1211.score.w.arranger = "SDedes/"
            currentLine[0] = str.substring(0, 2); //get book
            currentLine[1] = str.substring(3, 5); //get mode
            currentLine[2] = str.substring(6, 8); //get day
            currentLine[3] = str.substring(11,13); //get service 
            currentLine[4] = str.substring(14,str.substring(14).indexOf(".")+14); //get name of hymns
            ...
            currentLine[1] = replaceMode(currentLine[1]); //Final format for the month
            currentLine[2] = replaceDay(currentLine[2]);
            currentLine[3] = replaceService(dayDesc, currentLine[3], specialTr);
            currentLine[4] = replaceKey(keyDesc,currentLine[4], specialTr+serviceIdentifier);
            
            //sets the beginning of the tree
            //This will only get accessed for the first iteration that finds an "oc"
            if(firstLine) {
            	firstLine = false;
            ...
            for(int i = 0 ; i < currentLine.length ; i++) {
            	...
            	if(i==currentLine.length-1 && currentLine[i].equals(previousLine[i])) {
            		...
            		for(int j = 4 ; j >= i  ; j--) {	// if level 4 diff, then loop once, if level 3 diff, then loop twice, etc
            			fs += tabs(8+2*j)+"</ul>\n"+tabs(7+2*j)+"</li>\n";
            		}
        ...
        try {
            FileWriter myWriter = new FileWriter("C:/git/ages-alwb-system/net.ages.liturgical.workbench.system/MEDIA_INDEX_UTILITY/output/grOC.html");
            myWriter.write(fs);
            myWriter.close();
    ...
    public static String replaceKey(ArrayList<String> arr, String str, String id) { //str is level4 -- arr is key desciriptor
        ...
        for(int j = 0; j<arr.size(); j++){
            if(Pattern.matches("^oc"+id+".*[.]"+str+"[.].*",arr.get(j))){  //checking key descriptor
    ...
    public static String replaceMode(String s) {
    	return "Mode " + s.substring(1);
    }
    
    public static String replaceService(ArrayList<String> arr, String s, String tr) {
       
    	//create post fix if Triodion
    	if(tr.equals("tr")) {tr = " (in Triodion)";}else {tr="";}
...
//        if(s.equals("MA")) return "Matins" + tr;
//        if(s.equals("LI")) return "Liturgy" + tr;
//        if(s.equals("VE")) return "Vespers" + tr;
//        if(s.equals("H1")) return "First Hour" + tr;
//        if(s.equals("H3")) return "Third Hour" + tr;
//        if(s.equals("H6")) return "Sixth Hour" + tr;
//        if(s.equals("H9")) return "Ninth Hour" + tr;
//        return s;
        
    }
    
    public static String replaceDay(String d) {
        if(d.equals("d1")) return "Sunday";
        if(d.equals("d2")) return "Monday";
        if(d.equals("d3")) return "Tuesday";
        if(d.equals("d4")) return "Wednesday";
        if(d.equals("d5")) return "Thursday";
        if(d.equals("d6")) return "Friday";
        if(d.equals("d7")) return "Saturday";
        return d;
    }
